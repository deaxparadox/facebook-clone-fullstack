...
def login_api_view(request):
    ...
    if login_serializer.is_valid():
        ...
        # login invalid
        if not uid_and_token:
            return JsonResponse({}, status=status.HTTP_401_UNAUTHORIZED)
        
        return Response(uid_and_token, status=status.HTTP_202_ACCEPTED)
...
def create_user_api_view(request):
    ...
    # create account
    try:    
        account = Account.objects.create(user=user, apitoken=token, setting=setting, uid=unique)
